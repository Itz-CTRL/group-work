...
│   ├── models.py             # Database models (User, Transcript, Event, etc.)
│   ├── routes/
│   │   ├── admin.py          # Admin routes & APIs
│   │   │                      # Routes: /admin/login, /admin/dashboard
│   │   │                      # APIs: /api/users, /api/complaints/*, /api/transcripts/*, /api/notifications
│   │   └── student.py        # Student routes & APIs
│   │                          # Routes: /login, /signup, /logout, /dashboard, /profile, etc.
│   │                          # APIs: /api/complaints, /api/transcripts, /api/notifications, etc.
│   └── templates/
...
│       │   ├── settings.html
│       │   └── support.html
│       └── layouts/          # Shared template components
│           ├── _card_header.html   # Header for login/admin pages
│           └── _flash.html         # Flash messages display
├── data/                     # SQLite database files (created automatically)
├── .env                      # Environment variables (DO NOT COMMIT - use .gitignore)
├── .gitignore                # Git ignore rules (protects .env, __pycache__, venv, etc.)
...
- Virtual environment files (`venv/`, `env/`, `.venv`)
- IDE configuration files (`.vscode/`, `.idea/`)
- Database files (`*.db`, `*.sqlite`, `data/`)
- Log files and test coverage reports

## Database Schema

The application uses SQLite with the following main tables:
- `User` - Student and admin accounts with profiles
- `Complaint` - Student support tickets with status tracking
- `Transcript` - Academic transcript requests with status
- `Event` - Community events with dates and descriptions
- `Job` - Career opportunities and job postings
- `Donation` - Alumni contributions and donations
